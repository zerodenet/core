...
配置、控制 API、能力清单和稳定错误码按[稳定 V1 外部契约](./stable-contract-v1.md)独立发布版本范围。省略 `schema_version` 的历史配置按 V1 读取，导出时显式写入 V1；未知配置版本必须在运行时构造前失败。客户端以正向能力事实生成安全配置，并结合全局及协议限制码解释当前边界。
...
TCP 链式连接表示为流路径。UDP 链式连接表示为数据包路径。运行时可以缓存 socket、流、关联、响应任务和中性路由状态，但协议数据包编码、解码和响应封装保持在协议 crates 或中性协议特征中。
...
- `contracts`、`error_codes` 和配置 `schema_version` 与稳定 V1 文档及兼容测试一致；
