...
`AuthenticatedQuicInboundProfile` 负责 QUIC 载体接入；认证后的连接由 HY2 实现
`zero-core` 的 `InboundStreamMultiplexer` / `InboundDatagramMultiplexer` 契约。
通用运行时通过契约管理逻辑流任务和生命周期，不解释 HY2 帧或持有协议复用状态。
