...
这项策略同时作用于 TCP、managed UDP 和 packet-path；UDP 缓存标识包含节点安全与载体身份，
策略改变后不会复用不同策略的旧连接。`server_name` 可覆盖 TLS 的 SNI 与证书校验名称，缺省使用 `server`；拨号地址保持 `server`。
`ca_cert_path` 为节点级私有 CA 文件（相对配置文件目录解析），`tls_options.pinned_peer_cert_sha256`
接受 base64 SHA-256 证书 pin。CA、pin、客户端指纹、Salamander 和跳端口都纳入连接池身份。
