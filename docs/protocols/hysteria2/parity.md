...
| 类别 | 剩余工作 | 实现原则 |
| --- | --- | --- |
| 已有内核能力复用 | 业务限速、用户策略、路由、DNS、TUN、统计 | 继续使用现有内核接口；HY2 带宽协商不能替代 principal 共享限速，详见[速率语义](transport.md#与内核限速的关系) |
...
| 参数边界差异 | 非零带宽最小值 | 当前客户端、服务端均要求至少 65,536 字节/秒；参考版仅服务端有此下限，客户端兼容校验仍需拆分 |
