...
| QUIC 窗口、保活、PMTU、连接复用/恢复 | 自动接收扩窗、参数和 TCP 池已实现，有边界回归 | 固定/初始/最大窗口、RTT 驱动增长和流/连接协调见[窗口对齐](windows.md)；保活、PMTU；TCP 单次建连、并发复用、重建、重载和空闲回收；UDP 保留已有独立流缓存，长稳与跨流共享仍待验收/实现 |
...
下载后校验发布资产 SHA-256，再运行官方端基础四项、Brutal 双向扩展两项、受控 Brutal 丢包对照一项、三档 BBR 受控链路对照一项、接收窗口高时延对照一项和 sing-box 六项互通测试。
