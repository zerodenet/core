...
| 带宽配置、Brutal、BBR 配置 | 已实现，配置已统一 | Zero 只暴露 `up_bps/down_bps`；适配器映射协议收发参数；协议拥有协商与 Brutal，不再隐式附加 QUIC 硬上限或逻辑流预算；官方 BBR 三档仍未实现 |
...
下载后校验发布资产 SHA-256，再运行官方端基础四项、Brutal 双向扩展两项、受控丢包对照一项和 sing-box 六项互通测试。
