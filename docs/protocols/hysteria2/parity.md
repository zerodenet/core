...
| 拥塞与窗口行为差异 | 固定接收窗口与初始/最大自动扩窗；载体调度及长稳性能 | BBR 三档、采样与状态机由共享传输层执行；Quinn 与 quic-go 的调度并非逐包等价，自动接收扩窗及长稳另行验收 |
...
下载后校验发布资产 SHA-256，再运行官方端基础四项、Brutal 双向扩展两项、受控 Brutal 丢包对照一项、三档 BBR 受控链路对照一项和 sing-box 六项互通测试。
QUIC pacing 和逐包反馈扩展执行独立依赖单元回归；固定官方源码生成 BBR 状态向量并校验仓库结果。
