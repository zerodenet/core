...
同日，提交 `d2f3e159` 的[自动互通验收](https://github.com/zerodenet/core/actions/runs/34312975214)
使用 SHA-256 核验过的官方 v2.12.2 发布程序，通过官方 9 项、sing-box 6 项与 Quinn 268 项测试。
CI 从固定官方源码重新生成并核对 147 个窗口检查点和 252 个 BBR 检查点。

| 接收端 | 固定窗口耗时 | 自动窗口耗时 | 全程吞吐提升 |
| --- | --- | --- | --- |
| Zero | 29.461 s | 9.050 s | 3.25 倍 |
| 官方 | 32.202 s | 9.080 s | 3.55 倍 |

两个自动窗口场景的全程吞吐比为 1.003；链路条件与前述本地对照相同，全部预设门槛通过。
[工作区 CI](https://github.com/zerodenet/core/actions/runs/34312975137)通过 1507 项测试
（90 项显式忽略）、严格 Clippy、分层回归、三平台检查与 musl 构建。
外部互通和特权用例另行执行，不将其余忽略项计作已验收。
本地严格 Clippy、validation-only、窗口专项和载体全量回归通过；本地工作区全量运行在 CI
完成后停止，不记作本地全量通过。
