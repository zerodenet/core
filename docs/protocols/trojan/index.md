...
| WS/gRPC 载体 | `supported` | TLS 后封装 WebSocket 或 gRPC，入站/出站对称 |
...
外部互操作测试仍保留在 `crates/proxy/tests/trojan_xray_interop.rs` 并默认忽略；能力完整性的 CI 门由 Zero→Zero TCP/UDP/MUX 以及 WS/gRPC 端到端测试承担。
