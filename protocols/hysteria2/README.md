...
| TCP 请求头分段读取、合并到达的业务数据保留和有界 padding | 已有确定性回归测试 |
...
| 凭据热更新清退旧连接、拒绝旧密码并接受新密码 | 已用 sing-box 验证 |

真实外部二进制测试位于
`crates/proxy/tests/hysteria2_sing_box_interop.rs`，默认 `#[ignore]`，需要
`SING_BOX_BIN` 指向 sing-box 可执行文件。

2026-09-08 使用从 `github.com/sagernet/sing-box@v1.13.14` 构建的二进制
复验，上述 6 项外部测试全部通过；TCP 流读取的 11 项确定性回归位于
`tests/tcp_stream.rs`。
...
src/shared/stream.rs       TCP 字段精确读取与长度边界
