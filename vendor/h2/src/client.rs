...
#[derive(Clone, Debug)]
pub struct Builder {
    application_settings: Option<Vec<u8>>,
...
mod alps;
impl Builder {
    /// Supply authenticated peer ALPS bytes before opening any streams.
    /// Malformed ALPS fails the handshake. No SETTINGS acknowledgement is sent
    /// for ALPS; subsequent on-wire SETTINGS retain normal ACK semantics.
    pub fn peer_application_settings(&mut self, settings: &[u8]) -> &mut Self {
        self.application_settings = Some(settings.to_vec());
        self
    }
    ...
    B: Buf,
{
    async fn handshake2(
        ...
        if let Some(settings) = alps {
            inner.apply_alps(&settings)?;
        }
