    ...
    B: Buf,
{
    ...
    pub(crate) fn apply_alps(&mut self, settings: &[frame::Settings]) -> Result<(), Error> {
        // ALPS establishes the peer's initial settings without an HTTP/2 ACK.
        // Mark this before the wire preface so an omitted MAX_CONCURRENT_STREAMS
        // in the subsequent SETTINGS does not erase the negotiated ALPS value.
        self.inner
            .streams
            .apply_remote_settings(&frame::Settings::default(), true)?;
        self.inner
            .settings
            .mark_remote_initial_settings_as_received();
        for setting in settings {
            self.inner.streams.apply_remote_settings(setting, false)?;
            if let Some(size) = setting.header_table_size() {
                self.codec.set_send_header_table_size(size as usize);
            }
            if let Some(size) = setting.max_frame_size() {
