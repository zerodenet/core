...
pub struct Connection {
    ...
    incoming_bi: BoxStreamSync<'static, Result<(quinn::SendStream, quinn::RecvStream, Bytes), quinn::ConnectionError>>,
...
impl Connection {
    /// Use a protocol-dispatched incoming stream source. Prefix bytes are replayed
    /// before the remaining receive stream; uni/control streams remain on Quinn.
    pub fn with_incoming_bidi(
        conn: quinn::Connection,
        incoming: impl Stream<Item = Result<(quinn::SendStream, quinn::RecvStream, Bytes), quinn::ConnectionError>> + Send + Sync + 'static,
    ) -> Self {
        let mut connection = Self::new(conn);
        connection.incoming_bi = Box::pin(incoming);
        connection
    }
    ...
    pub fn new(conn: quinn::Connection) -> Self {
        Self {
            ...
            incoming_bi: Box::pin(stream::unfold(conn.clone(), |conn| async {
                Some((conn.accept_bi().await.map(|(send, recv)| (send, recv, Bytes::new())), conn))
            })),
            opening_bi: None,
    ...
    B: Buf,
{
    ...
    fn poll_accept_bidi(
        ...
        let (send, recv, prefix) = ready!(self.incoming_bi.poll_next_unpin(cx))
            .expect("self.incoming_bi BoxStream never returns None")
            .map_err(|e| convert_connection_error(e))?;
        Poll::Ready(Ok(Self::BidiStream {
            send: Self::SendStream::new(send),
            recv: Self::RecvStream { prefix: Some(prefix), ..Self::RecvStream::new(recv) },
        }))
    }
...
impl quic::RecvStream for RecvStream {
    ...
    fn poll_data(
        ...
        if let Some(prefix) = self.prefix.take().filter(|b| !b.is_empty()) { return Poll::Ready(Ok(Some(prefix))); }
