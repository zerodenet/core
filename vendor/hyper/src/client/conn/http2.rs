    ...
    Ex: Clone,
{
    ...
    /// Supply authenticated HTTP/2 ALPS settings before opening any streams.
    /// Parsing and application are delegated to h2; no ALPS SETTINGS ACK is sent.
    pub fn peer_application_settings(&mut self, settings: &[u8]) -> &mut Self {
        self.h2_builder.application_settings = Some(settings.to_vec());
        self
    }
