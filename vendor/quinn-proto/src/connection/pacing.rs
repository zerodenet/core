...
impl Pacer {
    ...
    pub(super) fn delay_with_rate(
        &mut self, smoothed_rtt: Duration, bytes_to_send: u64, mtu: u16,
        window: u64, rate: Option<u64>, now: Instant,
    ) -> Option<Instant> {
        let Some(rate) = rate else {
            return self.delay_default(smoothed_rtt, bytes_to_send, mtu, window, now);
        };
        let rate = rate.max(1);
        // Two milliseconds of pacing credit, bounded to a small packet burst.
        self.capacity = (rate / 500).clamp(2 * u64::from(mtu), 256 * u64::from(mtu));
        let earned = now.saturating_duration_since(self.prev).as_nanos()
            .saturating_mul(u128::from(rate)).saturating_add(self.fractional_tokens);
        self.fractional_tokens = earned % 1_000_000_000;
        let credit = (earned / 1_000_000_000).min(u128::from(u64::MAX)) as u64;
        self.tokens = self.tokens.saturating_add(credit).min(self.capacity);
        self.prev = now;
        if self.tokens >= bytes_to_send { return None; }
        let nanos = (u128::from(bytes_to_send - self.tokens) * 1_000_000_000).div_ceil(u128::from(rate));
        Some(now + Duration::from_nanos(nanos.min(u128::from(u64::MAX)) as u64))
    }

    fn delay_default(
        &mut self, smoothed_rtt: Duration, bytes_to_send: u64, mtu: u16,
        window: u64, now: Instant,
    ) -> Option<Instant> {
        debug_assert_ne!(
            window, 0,
