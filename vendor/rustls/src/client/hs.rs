...
fn emit_client_hello_for_retry(
    ...
    if let Some(factory) = &config.client_hello_profile {
        ...
        if config.ech_mode.is_none() && chp_payload.encrypted_client_hello.is_some() {
    ...
    input.hello.sent_extensions = chp_payload.wire_collect_used();
