...
extension_struct! {
    ...
    pub(crate) struct ClientExtensions<'a> {
        ...
        /// Connection-local presentation; live tickets, binders and keys are never stored here.
        pub(crate) wire_order: Option<Vec<ExtensionType>>,
        pub(crate) wire_extra: Vec<(u16, Vec<u8>)>,
...
impl ClientExtensions<'_> {
    ...
    pub(crate) fn wire_collect_used(&self) -> Vec<ExtensionType> {
        let mut used = self.collect_used();
        for (id, _) in &self.wire_extra {
    ...
    pub(crate) fn wire_clone_one(&mut self, other: &Self, ext: ExtensionType) {
        self.clone_one(other, ext);
        if let Some(value) = other
            .wire_extra
            .iter()
            .find(|(id, _)| *id == u16::from(ext))
        {
            self.wire_extra.push(value.clone());
        }
    }
    fn wire_encode_one(&self, ext: ExtensionType, bytes: &mut Vec<u8>) {
        if let Some((id, body)) = self.wire_extra.iter().find(|(id, _)| *id == u16::from(ext)) {
            id.encode(bytes);
    ...
    pub(crate) fn used_extensions_in_encoding_order(&self) -> Vec<ExtensionType> {
        if let Some(order) = &self.wire_order {
            if self.contiguous_extensions.is_empty() && self.encrypted_client_hello_outer.is_none()
            {
                let mut used = self.wire_collect_used();
                used.retain(|ext| *ext != ExtensionType::PreSharedKey);
                used.sort_by_key(|ext| order.iter().position(|id| id == ext).unwrap_or(usize::MAX));
                if self.preshared_key_offer.is_some() {
                    used.push(ExtensionType::PreSharedKey);
                }
                return used;
            }
        }
    ...
    fn order_insensitive_extensions_in_random_order(&self) -> Vec<ExtensionType> {
        let mut order = self.wire_collect_used();
        ...
        if let Some(preferred) = &self.wire_order {
            order.sort_by_key(|ext| {
                preferred
                    .iter()
                    .position(|id| id == ext)
                    .unwrap_or(usize::MAX)
...
impl<'a> Codec<'a> for ClientExtensions<'a> {
    fn encode(&self, bytes: &mut Vec<u8>) {
        ...
        for item in order {
            self.wire_encode_one(item, body.buf);
        }
    }
...
impl ClientHelloPayload {
    ...
    pub(crate) fn payload_encode(&self, bytes: &mut Vec<u8>, purpose: Encoding) {
        ...
        for e in &to_compress {
            compressed.clear(*e);
            compressed.wire_extra.retain(|(id, _)| *id != u16::from(*e));
        }
